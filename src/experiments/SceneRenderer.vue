...
<template><div class="scene-container"><BinarySceneView v-if="scene.kind === 'binary'" :scene="scene" @action="emit('action', $event)" /><CpuSceneView v-else-if="scene.kind === 'cpu'" :scene="scene" /><CacheSceneView v-else-if="scene.kind === 'cache'" :scene="scene" /><ProcessSceneView v-else-if="scene.kind === 'process'" :scene="scene" /><VirtualMemorySceneView v-else-if="scene.kind === 'virtual-memory'" :scene="scene" /><NetworkSceneView v-else-if="scene.kind === 'network'" :scene="scene" /><TreeSceneView v-else :scene="scene" /></div></template>
