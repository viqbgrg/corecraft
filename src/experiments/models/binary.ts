...
export function initialBinary(width = 8): BinaryState {
  return {
    ...
    b: 15,
    operation: 'AND',
    bitsDraft: (42).toString(2).padStart(width, '0'),
    error: '',
    log: [],
    observedLogic: false,
...
export function transitionBinary(state: BinaryState, action: ExperimentAction): BinaryState {
  ...
  if (action.type === 'a' || action.type === 'b')
    s[action.type] = integer(action.value, 0, mask, s[action.type])
  ...
  const detail =
    ...
    result +
    (carry
      ? '；完整和是 ' + full + '，第 ' + (s.width + 1) + ' 位的进位无法存入 ' + s.width + ' 位寄存器。'
      : '；结果由每一位的规则决定。')
...
export function presentBinary(s: BinaryState): ExperimentView {
  ...
  return {
    ...
    metrics: [
      { label: '十进制结果', value: result },
      { label: '十六进制', value: '0x' + result.toString(16).toUpperCase().padStart(2, '0') },
      { label: '无符号范围', value: '0–' + (2 ** s.width - 1) },
      { label: '进位 Carry', value: carry, tone: carry ? 'warning' : 'neutral' },
    ],
    status: {
      title: s.error
        ? '输入需要调整'
      ...
      detail:
        s.error ||
        (carry
          ? '8 位只保留低 8 位，所以 255 + 1 的存储结果是 0，进位为 1。这是无符号溢出。'
          : '点击 A 或 B 的任意一位，观察位权、十进制与运算结果如何一起变化。'),
      tone: s.error ? 'danger' : carry ? 'warning' : 'neutral',
    },
    log: s.log,
    goal: { label: '尝试位运算，再制造一次 8 位加法溢出。', reached: s.observedLogic && s.observedOverflow },
  }
}
export const binaryEngine: EngineFactory = (config) => {
  if (config.width !== undefined && config.width !== 8)
    throw new Error('The binary lab currently supports width: 8 only')
  return createSession(() => initialBinary(8), transitionBinary, presentBinary)
}
