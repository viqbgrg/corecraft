...
interface Comparison { rate: number; cycles: number; ram: number }
...
export function initialCache(): CacheState {
  return { draft: '0x1000', current: null, l1: Array<number | null>(4).fill(null), l2: Array<number | null>(8).fill(null), l1Hits: 0, l2Hits: 0, ram: 0, cycles: 0, source: null, recent: [], comparisons: {}, log: [], error: '' }
}
export function accessCache(state: CacheState, value: number): CacheState {
  ...
  let detail = ''
  if (s.l1[l1Index] === block) { s.l1Hits++; detail = 'L1 Hit：同一条 16 B Cache Line 已在 L1。' }
  ...
  s.log = addLog(s.log, hex(value) + ' · ' + source, detail + ' 模拟成本 ' + cycles + ' 周期。', source === 'RAM' ? 'warning' : 'success')
  return s
}
...
export function transitionCache(state: CacheState, action: ExperimentAction): CacheState {
  ...
  if (action.type === 'sequential' || action.type === 'shuffled') {
    ...
    s.log = addLog(state.log, '冷缓存对照 · ' + (action.type === 'sequential' ? '顺序' : '打乱'), '清空两级缓存与本组计数；访问完全相同的 32 个地址，仅改变顺序。')
...
export function presentCache(s: CacheState): ExperimentView {
  ...
  return {
    scene: { kind: 'cache', address: s.current, lineSize: 16, path: s.source, levels: [{ name: 'L1 Cache', latency: 1, lines: s.l1.map((blockId, index) => ({ index, block: blockId, active: blockId !== null && blockId === block })) }, { name: 'L2 Cache', latency: 8, lines: s.l2.map((blockId, index) => ({ index, block: blockId, active: blockId !== null && blockId === block })) }], recent: s.recent },
    controls: [
      { id: 'address', kind: 'text', label: '内存地址', value: s.draft },
      { id: 'quick-address', kind: 'select', label: '快速访问', value: '', options: [{ value: '', label: '选择一个地址' }, ...[0x1000, 0x1004, 0x1008, 0x100c, 0x1040, 0x2000].map(v => ({ value: hex(v), label: hex(v) }))] },
    ...
    metrics: [
      { label: 'L1 Hit Rate', value: rate, unit: '%' }, { label: 'L2 Hits', value: s.l2Hits }, { label: 'Memory Access', value: s.ram, unit: '次' }, { label: '本组模拟成本', value: s.cycles, unit: '周期' },
      ...(s.comparisons.sequential ? [{ label: '顺序组 · L1 命中', value: s.comparisons.sequential.rate.toFixed(1) + '% / ' + s.comparisons.sequential.cycles + ' 周期' }] : []),
      ...(s.comparisons.shuffled ? [{ label: '打乱组 · L1 命中', value: s.comparisons.shuffled.rate.toFixed(1) + '% / ' + s.comparisons.shuffled.cycles + ' 周期' }] : []),
    ],
    status: { title: s.error ? '地址无效' : s.source ? (s.source === 'RAM' ? 'Cache Miss：取回的不只是一个数。' : s.source + ' Hit：数据就在附近。') : '先访问 0x1000，再试试 0x1004。', detail: s.error || s.log.at(-1)?.detail || '本模型为直接映射、包含式两级只读缓存。延迟为教学参数；一个 Cache Line 包含相邻 16 字节。', tone: s.error ? 'danger' : s.source === 'RAM' ? 'warning' : 'neutral' },
    log: s.log,
    goal: { label: '运行顺序与打乱两组，比较相同地址集合的命中率和成本。', reached: !!s.comparisons.sequential && !!s.comparisons.shuffled },
  }
}
export const cacheEngine: EngineFactory = () => createSession(initialCache, transitionCache, presentCache)
