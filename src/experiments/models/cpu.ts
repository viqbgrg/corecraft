...
type Instruction =
  ...
  | { op: 'ADD'; target: Register; source: Register }
  | { op: 'STORE'; address: number; source: Register }
  | { op: 'LOAD'; target: Register; address: number }
  | { op: 'HALT' }
type WriteBack =
  | { kind: 'register'; register: Register; value: number }
  | { kind: 'memory'; address: number; value: number }
  | { kind: 'halt' }
  | null
...
function program(a: number, b: number, example: string): Instruction[] {
  const code: Instruction[] = [
    { op: 'MOV', target: 'R1', value: a },
    { op: 'MOV', target: 'R2', value: b },
    { op: 'ADD', target: 'R1', source: 'R2' },
  ]
  if (example === 'memory')
    code.push({ op: 'STORE', address: 16, source: 'R1' }, { op: 'LOAD', target: 'R3', address: 16 })
  return [...code, { op: 'HALT' }]
}
...
export function cpuStep(state: CpuState): CpuState {
  ...
  if (stage === 'Fetch') {
    ...
    const i = s.ir
    if (i.op === 'MOV') {
      s.pending = { kind: 'register', register: i.target, value: i.value }
      s.alu = '直通 ' + i.value
    }
    if (i.op === 'ADD') {
    ...
    if (i.op === 'STORE') {
      s.pending = { kind: 'memory', address: i.address, value: s.registers[i.source] }
      s.alu = '地址 ' + hex(i.address, 2)
    }
    if (i.op === 'LOAD') {
      s.pending = { kind: 'register', register: i.target, value: s.memory[i.address] ?? 0 }
      s.alu = '读取 ' + hex(i.address, 2)
    }
    if (i.op === 'HALT') {
      s.pending = { kind: 'halt' }
      s.alu = '停止信号'
    ...
    detail =
      p?.kind === 'register'
        ? p.register + ' ← ' + p.value
        : p?.kind === 'memory'
          ? 'Memory[' + hex(p.address, 2) + '] ← ' + p.value
          : '程序结束。'
  }
  s.cycles += 1
...
export function transitionCpu(s: CpuState, action: ExperimentAction): CpuState {
  ...
  if (action.type === 'example' && ['arithmetic', 'memory'].includes(String(action.value)))
    return initialCpu(s.a, s.b, String(action.value))
  return s
}
export function presentCpu(s: CpuState): ExperimentView {
  return {
    scene: {
      ...
      phase: s.phase,
      pc: s.pc,
      activeInstruction: s.active,
      registers: s.registers,
      instruction: s.ir ? format(s.ir) : '尚未取指',
      alu: s.alu,
      program: s.program.map(format),
      memory: Object.entries(s.memory).map(([a, value]) => ({ address: Number(a), value })),
    ...
    controls: [
      ...
      { id: 'b', kind: 'number', label: 'MOV R2 的值', value: s.b, min: 0, max: 255, disabled: s.cycles > 0 },
      {
        ...
        value: s.example,
        options: [
          { value: 'arithmetic', label: '寄存器加法' },
          { value: 'memory', label: '加法 → 存储 → 读取' },
        ],
        disabled: s.cycles > 0,
      },
      {
        id: 'step',
        kind: 'button',
        label: s.phase === 'Halted' ? '执行结束' : '单步 · ' + s.phase,
        primary: true,
        disabled: s.phase === 'Halted',
      },
      { id: 'instruction', kind: 'button', label: '执行完当前指令', disabled: s.phase === 'Halted' },
    ],
    metrics: [
      { label: 'Program Counter', value: s.pc },
      { label: '已完成指令', value: s.retired },
      { label: '教学微步骤', value: s.cycles },
      { label: 'R1', value: s.registers.R1 },
    ],
    status: {
      title: s.phase === 'Halted' ? '指令变成了状态变化。' : '下一步：' + s.phase,
      detail: s.log.at(-1)?.detail ?? '先取指，再译码、执行、写回。点击单步，观察寄存器究竟在哪一步改变。',
      tone: s.phase === 'Halted' ? 'success' : 'neutral',
    },
    log: s.log,
    goal: { label: '逐步运行整个程序，观察 ADD 的执行与写回。', reached: s.phase === 'Halted' },
  }
}
export const cpuEngine: EngineFactory = (config) =>
  createSession(
    () =>
      initialCpu(
        integer(typeof config.a === 'number' ? config.a : 10, 0, 255, 10),
        integer(typeof config.b === 'number' ? config.b : 20, 0, 255, 20),
        config.example === 'arithmetic' ? 'arithmetic' : 'memory',
      ),
    transitionCpu,
    presentCpu,
  )
