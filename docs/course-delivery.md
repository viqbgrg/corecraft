...
- [x] Level 0 学习方法：从输入、处理、暂存与输出认识数据、状态和程序；中文规则实验、预测与模型边界。
...
在已有实验和主题覆盖之上，以下 20 节课补齐了从概念到操作的讲解。每课都提供具体输入下的推导、关键状态或操作对照、能够达成实验目标的步骤、预期观察、选做变化及课后自检；保留各自的模型边界，并按阅读与操作量更新预计学习时间。

| 分组           | 课程                                                             | 补充重点                                                                  |
| -------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------- |
| 数据表示       | Binary、Logic                                                    | 位权、掩码、进位、有限位宽；逐门求值、德摩根反例与完整真值表              |
| CPU 与操作系统 | CPU、Cache、Process、Virtual Memory、Page Replacement            | 执行与写回、地址映射与缓存成本、线程现场、缺页重试、FIFO / LRU 与容量反例 |
| 网络           | TCP Handshake、TCP Close、DNS、HTTP                              | 双端状态与序号、半关闭、正负缓存与 TTL、URL、连接复用及分层故障           |
| 数据结构       | Linear Storage、Stack / Queue、Hash Table、Trees / Heaps、B+Tree | 定位与修改成本、环形槽位、墓碑与更新、整棵子树约束、分裂 / 借位 / 合并    |
| 算法           | Binary Search、Sorting、Graph、Dynamic Programming               | 区间推导、稳定性反例、距离松弛与前驱、DP 状态表和无解判断                 |

讲义中的操作对应现有控件，数值示例以实际模型运行核对。基础实验与选做变化分开说明；修改输入、重置或切换模式时，明确哪些证据需要重新建立。正文表格、代码示例和小节统一支持窄屏换行及明暗主题。
