...
每个工厂接收只读 config。CPU 支持初始操作数 a / b 与 arithmetic / memory 示例；Binary 和 Cache 目前固定为 8 位与 16 B 行，会拒绝不支持的尺寸，避免定义与模型悄悄不一致。其他模型目前使用空配置。扩展参数时，要同时更新工厂校验、教学边界和针对性的测试。
