...
入门课程用中文步骤展示输入、处理、暂存与输出，只选择规则和参数，不要求编写指令。逻辑课程使用专用布尔表达式解析器，不执行用户脚本。binary32 模型在内部用 BigInt 有理数处理精确舍入，公开的 ExperimentView 只暴露字符串和数值。文件系统、页缓存与 IO 模型各自维护资源引用与事件状态，仍复用 DataScene，不共享可变状态。
